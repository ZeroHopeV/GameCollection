...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        if ($check_stmt->rowCount() > 0) {
            ...
            $update_query = "UPDATE run_forever SET normalhighscore = :normalhighscore WHERE run_forever_id = :user_id;";
            // Prepares the query
            $update_stmt = $pdo->prepare($update_query);
            // Binds the parameters
            $update_stmt->bindParam(":normalhighscore", $_POST["normalhighscore"]);
            $update_stmt->bindParam(":user_id", $_SESSION["user_id"]);
            // Executes the query
            $update_stmt->execute();

        } else {
            // Inserts normalhighscore into run_forever with a specific user-id
            $insert_query = "INSERT INTO run_forever (run_forever_id, easyhighscore, normalhighscore, hardhighscore) VALUES (:user_id, 0, :normalhighscore, 0);";
            // Prepares the query
            $insert_stmt = $pdo->prepare($insert_query);
            // Binds the parameters
            $insert_stmt->bindParam(":user_id", $_SESSION["user_id"]);
            $insert_stmt->bindParam(":normalhighscore", $_POST["normalhighscore"]);
            // Executes the query
            $insert_stmt->execute();
        }
